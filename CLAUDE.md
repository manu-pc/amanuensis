...
- **The `* ` dialogue prefix can be glued to any marker**, `~n` included: `~1* Texto` and `\E~1* Texto` are 767 lines of the corpus (all at the head of a visual line, none mid-line). The tokenizer consumes it as a fixed `FORMAT` after `\cX`, `\En` *and* `~n`; read as the color placeholder instead, the line's first `\cX` ends up where the prefix was. A prefix sitting behind leading relocatables is re-anchored per visual line (`MarkerReapplier.shiftLead`), so splitting `\cp* …` in two emits `* ` at the start of the new line rather than one character into it.
- **`~n` is not one thing.** It is the mod's runtime substitution (`* ~1 perdoa a ~2!`, `AT:~1 DF:~2`, `$~1`) — relocatable, placeholder `~`. But in the `~1* Texto` family it is two other things, and both are now tokenized as such: the leading `~n` sits in the **expression slot** where every other line has a `\EX`, so it is a fixed `FORMAT` the translator never sees; and a later `~2` in that same line is the **game's line break**, tokenized as `NEWLINE` so it shows as a real newline in the edit box and the translator just presses Enter. Evidence, over all five chapters: inside that family every non-leading `~` is `~2` (539 of them, 0 exceptions), and the visible run before each measures 13–38 characters — one dialogue line's width. Outside the family nothing changed. A line-break `~` takes **one** digit only: `more than~20 damage` is a break plus `0 damage`, and greedy digits would eat the `0` out of the translator's text.
